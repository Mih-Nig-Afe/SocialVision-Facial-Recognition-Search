# SocialVision Documentation Index

Central knowledge base for the SocialVision research project. Use this index to jump to the right playbook before coding, testing, or demoing the system.

---

## Documentation Map

### Core References

1. **[PROJECT_STATUS.md](PROJECT_STATUS.md)** – Executive summary, completion metrics, risks, KPIs.
2. **[CURRENT_CAPABILITIES.md](CURRENT_CAPABILITIES.md)** – Quick feature reference, usage snippets, limitations.
3. **[TESTING_GUIDE.md](TESTING_GUIDE.md)** – Automated + manual test recipes, troubleshooting trees.
4. **[DEVELOPMENT_ROADMAP.md](DEVELOPMENT_ROADMAP.md)** – Phase definitions, milestones, priority backlog.
5. **[SocialVision_Technical_Proposal.md](SocialVision_Technical_Proposal.md)** – Full architecture and research narrative.

### Deployment & Demo Guides

- **[DOCKER_TESTING_GUIDE.md](DOCKER_TESTING_GUIDE.md)** – BuildKit workflow, Compose profiles, GPU notes, common errors.
- **[DEMONSTRATION_GUIDE.md](DEMONSTRATION_GUIDE.md)** – End-to-end demo scripts and talking points.

---

## Suggested Reading Paths

- **New developers:** `PROJECT_STATUS` → `DEVELOPMENT_ROADMAP` → `TESTING_GUIDE` → source tree.
- **QA/testers:** `CURRENT_CAPABILITIES` → `TESTING_GUIDE` → log locations noted in `DEMONSTRATION_GUIDE`.
- **Stakeholders/demo prep:** `DEMONSTRATION_GUIDE` + `PROJECT_STATUS` for the latest metrics.

---

## Living Document Tracker

| Document | Status | Last Updated |
|----------|--------|--------------|
| PROJECT_STATUS.md | 🔄 In Sync | Nov 2025 |
| CURRENT_CAPABILITIES.md | 🔄 In Sync | Nov 2025 |
| TESTING_GUIDE.md | ✅ Stable | Dec 2024 |
| DOCKER_TESTING_GUIDE.md | ✅ Stable | Dec 2024 |
| DEMONSTRATION_GUIDE.md | ✅ Stable | Dec 2024 |
| DEVELOPMENT_ROADMAP.md | ✅ Stable | Dec 2024 |
| Technical Proposal | ✅ Stable | Jun 2025 |
| API Documentation | ⏳ Planned | TBA |
| Installation Guide | 🔄 Covered in root README | Nov 2025 |

Dates reflect the last substantive edit, not formatting changes. If you touch a document, bump the date and summarize the change at the top of that file.

---

## Contact

Questions or requests for documentation updates:

- Mihretab N. Afework – <mtabdevt@gmail.com>
- GitHub – [@Mih-Nig-Afe](https://github.com/Mih-Nig-Afe)

---

Last updated: November 2025

